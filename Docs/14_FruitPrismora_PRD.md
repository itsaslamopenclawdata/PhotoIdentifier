...
| Field | Details |
|---|---|
| **Document Title** | FruitPrismora — Product Requirements Document |
| **Version** | 1.0 |
...
| NutriPrismora Integration Active Users | 30% of NutriPrismora users | 50% of NutriPrismora users |
...
| Nutritional Profile | NutriPrismoraries, protein, carbs, fat, fiber, vitamins (A/B/C/E/K), minerals | USDA FoodData Central sourced |
...
| Daily Fruit Log | Track daily fruit consumption; view totals | Integrates with NutriPrismora via shared nutrition API |
...
| Cross-App Nutrition Sync | Auto-log fruit nutrition to NutriPrismora daily diary on scan | Internal API; user opt-in per fruit scan |
...
- **US-202:** As a NutriPrismora user, I want my fruit scan to automatically add to my daily nutrition diary.
...
│  PostgreSQL │ Redis │ S3 │ NutriPrismora Internal API     │
└────────────────────────────────────────────────────┘
```
...
**FruitLog**  *(linked to NutriPrismora MealLog)*
```
log_id             UUID PK
user_id            UUID FK
fruit_id           UUID FK
...
logged_at          TIMESTAMP
calo_log_id        UUID FK (nullable; set if pushed to NutriPrismora)
```
...
#### POST `/api/v1/log` 
Log fruit to daily intake; optionally push to NutriPrismora.
...
3. **Daily Log** — Fruit intake chart; sync indicator with NutriPrismora
...
### Design System
- **Font:** Nunito (400/700) — fresh, approachable
- **Color:** Mango Orange `#FF8C00` primary, Leaf Green `#52B788` accent, Cream `#FEFAE0` background
...
| NutriPrismora (Internal API) | NutriPrismorarie log synchronization |
...
- NutriPrismora integration for nutrition sync
...
*Document Version 1.0 — FruitPrismora PRD — PhotoIdentifier Platform | 2026-02-21*
