# MASTER PRD: PhotoIdentifier Prismora Platformlication
...
| Field | Details |
|---|---|
| **Document Title** | PhotoIdentifier Prismora Platformlication — Master Platform PRD |
| **Version** | 1.0 |
...
The **PhotoIdentifier Prismora Platformlication** is a unified, AI-first identification and intelligence platform integrating 17 specialized sub-applications under one authenticated, single-sign-on Progressive Web Application. Users access a curated suite of AI identification engines — spanning numismatics, fitness, nature, food, vehicles, pets, and collectibles — through a single account, shared data layer, and coherent design system.

The platform's singular competitive advantage is **cross-app AI synergy**: a user's activity in one sub-app enriches intelligence in others. A NutriPrismora user's nutrition goals inform FitPrismora's workout recovery recommendations. A FloraPrismora user's toxicity scan triggers a relevant alert in AquaPrismora when an identified toxic plant is common in their region. The unified data model enables personalization loops that no single-domain app can replicate.
...
║   ┌──────────────────────────────────────────────────────────┐    ║
║   │  Sub-App Shell: CoinPrismora | FitPrismora | VinylPrismora | NutriPrismora.. │    ║
...
|---|---|---|---|
| 01 | CoinPrismora | CoinPrismora AI | Numismatics |
| 02 | FitPrismora | FitPrismora | Beginner Fitness |
| 03 | VinylPrismora | VinylVault AI | Record Collecting |
| 04 | NutriPrismora | NutriLens AI | Food & Nutrition |
| 05 | RockPrismora | GeoLens AI | Geology & Gemology |
| 06 | ShroomPrismora | ForageSafe AI | Mycology & Foraging |
| 07 | MusclePrismora | MusclePrismora | Advanced Fitness |
| 08 | InsectPrismora | InsectPrismora | Entomology |
| 09 | BirdPrismora | BirdPrismora Pro | Ornithology |
| 10 | NotePrismora | NotePrismora | Banknote Collecting |
| 11 | AquaPrismora | AquaPrismora Pro | Aquarium Keeping |
| 12 | CardPrismora | CardPrismora AI | Sports Cards |
| 13 | FloraPrismora | FloraLens AI | Botany & Gardening |
| 14 | FruitPrismora | FruitPrismora | Fruit & Nutrition |
| 15 | AutoPrismora | AutoPrismora AI | Automotive |
| 16 | PawPrismora | PawPrismora AI | Canine Intelligence |
| 17 | MeowPrismora | MeowPrismora AI | Feline Intelligence |
...
- **"Unified Consistency"** — standardized design tokens create a cohesive platform experience while allowing sub-app brand expression
...
| Token | Value | Usage |
|---|---|---|
| **Font Family** | Inter | Universal across all platform UI elements |
| **Font Weights** | 300 (Light), 400 (Regular), 500 (Medium), 600 (SemiBold), 700 (Bold) | Universal weight scale |
| **Heading Scale** | H1: 32px/700, H2: 24px/600, H3: 20px/600, H4: 16px/600 | Consistent heading hierarchy |
| **Body Scale** | Body Large: 16px/400, Body: 14px/400, Body Small: 12px/400 | Content hierarchy |
| **Line Height** | Headings: 1.2, Body: 1.5 | Optimized readability |

#### Colors
| Token Category | Token Name | Value | Usage |
|---|---|---|---|
| **Primary** | `primary-50` | #EEF2FF | Background tints |
...
| | `primary-500` | #6366F1 | Primary brand color (Indigo) |
| | `primary-600` | #4F46E5 | Hover/pressed states |
| | `primary-700` | #4338CA | Active/focused states |
| | `primary-800` | #3730A3 | Dark overlays |
| | `primary-900` | #312E81 | Darkest accents |
| **Semantic** | `success` | #10B981 | Success states, positive feedback |
...
| Token | Value | Usage |
|---|---|---|
| `radius-xs` | 4px | Small elements, tags, chips |
| `radius-sm` | 8px | Input fields, small buttons |
| `radius-md` | 12px | Cards, modals, containers |
| `radius-lg` | 16px | Large cards, featured content |
| `radius-xl` | 24px | Pill buttons, hero banners |
...
|---|---|---|
| `space-0` | 0px | None |
| `space-1` | 4px | Base unit (0.25rem) |
| `space-2` | 8px | Small gaps (0.5rem) |
| `space-3` | 12px | Compact spacing (0.75rem) |
| `space-4` | 16px | Standard spacing (1rem) |
| `space-5` | 20px | Medium spacing (1.25rem) |
| `space-6` | 24px | Large spacing (1.5rem) |
| `space-8` | 32px | XL spacing (2rem) |
| `space-10` | 40px | XXL spacing (2.5rem) |
| `space-12` | 48px | Section padding (3rem) |
| `space-16` | 64px | Large sections (4rem) |

#### Shadows
| Token | CSS Value | Usage |
|---|---|---|
| `shadow-xs` | `0 1px 2px 0 rgba(0, 0, 0, 0.05)` | Subtle elevation |
| `shadow-sm` | `0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px -1px rgba(0, 0, 0, 0.1)` | Cards, small elements |
...
| `shadow-xl` | `0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)` | Hero elements, featured cards |
...
| `easing-in-out` | `cubic-bezier(0.4, 0, 0.6, 1)` | Bidirectional animations |

### 6.3 Platform vs. Sub-App Brand Expression
| Category | Platform Level | Sub-App Level |
|---|---|---|
| **Primary Color** | Indigo #6366F1 | Each sub-app has unique accent color for brand identity |
| **Typography** | Inter (universal) | Inter + optional brand font for headings |
| **Border Radius** | Unified scale | Must use token scale |
| **Spacing** | 4px base unit | Must use token scale |
| **Shadows** | 5-level system | Must use token scale |

### 6.4 Universal UX Components
...
|---|---|---|
| **Nutrition Sync** | NutriPrismora ↔ FruitPrismora | Fruit scan auto-logs to NutriPrismora daily diary |
| **Workout × NutriPrismoraries** | FitPrismora / MusclePrismora ↔ NutriPrismora | NutriPrismorarie burn from workouts deducted from NutriPrismora daily budget |
| **Beginner → Advanced Pathway** | FitPrismora → MusclePrismora | AI detects when FitPrismora user has graduated to intermediate level and suggests MusclePrismora |
| **Plant × Fish Safety** | FloraPrismora ↔ AquaPrismora | If plant in user's home is toxic to fish species in their AquaPrismora tank, alert is triggered |
| **Pet × Nutrition** | Dog/MeowPrismora ↔ NutriPrismora | Dietary needs of identified breed push guided nutrition log to NutriPrismora for the owner |
| **Wild Foraging Safety** | ShroomPrismora ↔ InsectPrismora | Combined foraging outing log with safety summary for both fungi and insects found |
...
| `coach` | MusclePrismora/FitPrismora coaching dashboard + client management |
...
- Sub-apps launched: CoinPrismora, NutriPrismora, FloraPrismora, FitPrismora, PawPrismora
...
- Sub-apps added: VinylPrismora, MusclePrismora, BirdPrismora, ShroomPrismora, RockPrismora, FruitPrismora, AquaPrismora
- Platform Pass subscription tier launched ($24.99/mo)
- Cross-app feature: Nutrition Sync (NutriPrismora ↔ FruitPrismora)
- Cross-app feature: Workout × NutriPrismoraries (FitPrismora/MusclePrismora ↔ NutriPrismora)
...
- Sub-apps added: NotePrismora, CardPrismora, PictureInsect, VehicleIdentifier, CatBreedIdentifier
...
*Document Version 1.0 — Prismora Platformlication Master PRD — PhotoIdentifier Platform | 2026-02-21*
