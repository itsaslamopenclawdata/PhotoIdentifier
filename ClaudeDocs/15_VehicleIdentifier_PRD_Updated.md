...
| Field | Details |
|---|---|
| **Document Title** | Vehicle Identifier — Product Requirements Document |
| **Version** | 1.0 |
...
- **O1:** Establish AutoLens AI as the most data-rich consumer vehicle intelligence tool
...
| EV Range Calculator | For electric vehicles: real-world range with weather correction | EPA range x temperature correction algorithm (based on ambient temp) |
...
| Repair Cost Regression | XGBoost model mapping panel x severity -> repair cost estimate |
...
### Design System
- **Font:** Inter (700 for headers, 400 for body text)
- **Color Palette:**
  - **Primary:** Indigo `#6366F1` — primary actions, brand elements, key CTAs
  - **Background:** White `#FFFFFF` — primary surface; Light Gray `#F9FAFB` — secondary background; Medium Gray `#F3F4F6` — tertiary background
  - **Text:** Dark Gray `#111827` — primary text; Medium Gray `#6B7280` — secondary text
  - **Borders:** Light Gray `#E5E7EB` — default borders
  - **Semantic:**
    - Success Green `#10B981` — positive indicators, no recalls
    - Warning Amber `#F59E0B` — warning states, moderate damage
    - Error Red `#EF4444` — error states, severe damage, recall alerts
    - Info Blue `#3B82F6` — informational states, EV and tech indicators
- **Border Radius:** 12px for cards, 8px for buttons, 4px for badges and small elements
- **Spacing:** 4px base unit; multiples of 4px (8px, 12px, 16px, 24px, 32px, 48px)
- **Shadows:**
  - Level 1 (subtle): `0 1px 2px 0 rgb(0 0 0 / 0.05)` — cards, buttons
  - Level 2 (light): `0 1px 3px 0 rgb(0 0 0 / 0.1), 0 1px 2px -1px rgb(0 0 0 / 0.1)` — dropdowns
  - Level 3 (medium): `0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)` — modals
  - Level 4 (strong): `0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)` — popovers
  - Level 5 (heavy): `0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)` — tooltips
- **Vehicle Spec Table:** Comparison bars show this vehicle vs. class average
- **Damage Map:** SVG car blueprint overlay; damage zones use semantic colors (amber for moderate, red for severe)
...
*Document Version 1.0 — Vehicle Identifier PRD — PhotoIdentifier Platform | 2026-02-21*
